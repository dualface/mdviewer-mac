...
Release artifacts are named as `MarkdownViewer-<version>-<build-number>-macOS.dmg`. The default marketing version is `0.5`; override it with `--version 0.5.1` or `MDVIEWER_VERSION`. The default release tag is `v<version>-<build-number>`.
